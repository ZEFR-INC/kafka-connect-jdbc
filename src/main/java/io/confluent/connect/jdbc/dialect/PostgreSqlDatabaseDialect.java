...
public class PostgreSqlDatabaseDialect extends GenericDatabaseDialect {
  ...
  public String addFieldToSchema(
    ...
    switch (columnDefn.type()) {
      ...
      case Types.OTHER: {
        ...
        if (isStringConvertibleType(columnDefn.typeName())) {
          builder.field(
              fieldName,
    ...
    switch (columnDefn.type()) {
      ...
      case Types.ARRAY: {
        return rs -> {
          ...
          boolean isStringConvertible = isStringConvertibleType(elementTypeName);
          ...
          for (Object obj: objectArray) {
            ...
            } else if (String.class.isAssignableFrom(obj.getClass()) || isStringConvertible) {
              stringArray.add(obj.toString());
            } else {
              throw new IOException(
                  "Can't process input, supported types in arrays are string, JSON, UUID, and null."
                  + " Your type: " + obj.getClass() + ", your array type: " + elementTypeName
              );
            }
          }
  ...
  protected boolean isStringConvertibleType(String typeName) {
    return JSON_TYPE_NAME.equalsIgnoreCase(typeName)
        || JSONB_TYPE_NAME.equalsIgnoreCase(typeName)
        || UUID_TYPE_NAME.equalsIgnoreCase(typeName);
  }
