  ...
  public SourceRecord extractRecord() throws SQLException {
    ...
    for (FieldSetter setter : schemaMapping.fieldSetters()) {
      try {
        ...
        log.warn("Ignoring record because processing failed in table " + tableId.tableName()
                 + " for field " + setter.toString() + ": ", e);
      } catch (SQLException e) {
        log.warn("Ignoring record due to SQL error in table " + tableId.tableName()
                 + " for field " + setter.toString() + ": ", e);
      }
    }
